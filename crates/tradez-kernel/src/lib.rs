        ...
        if let InboxMessage::External(data) = msg {
            ...
            match message {
                KernelMessage::PlaceOrder(order) => {
...

                    {
                        ...
                        match order.side {
                            Side::Ask => {
                                let balance =
                                    caller_account.balances.entry(Currencies::XTZ).or_insert(0);
                            ...
                            Side::Bid => {
                                let balance =
                                    caller_account.balances.entry(Currencies::USDC).or_insert(0);
                    ...
                    for event in events {
                        match event {
                            ...
                            Event::Trade {
                                ...
                                taker_total_filled =
                                    taker_total_filled.saturating_add(handle_trade_event(
                                        host,
                                        &mut accounts,
                                        order.side,
                                        order.price,
                ...
                KernelMessage::CancelOrder(cancel_order) => {
                    ...
                    } else if let Some(order) = orderbook.get_order(cancel_order.order_id) {
                        match order.side {
                            Side::Ask => {
                            ...
                            Side::Bid => {
                                if let Some(refund) = quote_value(order.remaining, order.price) {
                                    ...
                                    *balance = balance.checked_add(refund).unwrap();
                                    ...
                                    host.write_debug(
                                        "Failed to compute refund for cancelled bid order\n",
                                    );
                                }
                            }
                        }
